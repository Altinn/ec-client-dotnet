...
using EC_Endpoint_Client.SOArchiveStreamedEC2;

namespace EC_Endpoint_Client.Functionality.EndPoints.Archive
{
    public class ServiceOwnerArchiveEndpointFunctionalityEC2 : EndPointFunctionalityBase
    {
        #region ServiceOwnerArchiveExternalEC
        private ServiceOwnerArchiveExternalEC2Client GenerateSoArchiveProxy(string selectedEndpointName, X509Certificate2 selectedCertificate)
        {
            return
                GenerateProxy<ServiceOwnerArchiveExternalEC2Client, IServiceOwnerArchiveExternalEC2>(
        ...
        private ServiceOwnerArchiveExternalStreamedEC2Client GenerateSoArchiveStreamedProxy(string selectedEndpointName, X509Certificate2 selectedCertificate)
        {
            return
                GenerateProxy<ServiceOwnerArchiveExternalStreamedEC2Client, IServiceOwnerArchiveExternalStreamedEC2>(
                    selectedEndpointName, selectedCertificate);
        }
